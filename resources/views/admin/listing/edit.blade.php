...
<div class="main-content py-5">
    <div class="container">
        ...
        <div class="card shadow-lg border-0" style="border-radius: 15px; overflow: hidden;">
            <div class="card-body p-0">
                <form action="{{ route('admin.listing.update', $listing->id) }}" method="POST" enctype="multipart/form-data" id="listingForm">
                    @csrf
                    @method('PUT')
    ...
    <select name="owner_id" id="owner_id" class="form-select form-select-lg" required>
        @foreach($owners as $owner)
            <option value="{{ $owner->id }}" {{ $listing->owner_id == $owner->id ? 'selected' : '' }}>
